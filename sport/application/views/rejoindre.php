...
<body>
  <?php include( 'header.php'); echo '<p> Bonjour ' . $_SESSION[ 'login'] . '</p>'; ?>
  ...
  <div id="register">
    ...
    <form action="<?php echo site_url('Data/joinEquipe'); ?>" method="POST">
      <select name="name">
        ...
        <?php $connexion=mysqli_connect( 'dwarves.iut-fbleau.fr', 'duboisc', '60f36d1201', 'duboisc');
         $req=mysqli_query($connexion, 'SELECT DISTINCT nom FROM Equipe'); 
         $a=0 ; 
         foreach ($req as $equipe) { 
          echo '<option value=' .$a. '>'.$equipe[ 'nom']. '</option>'; $a++; 
        } 
          ?>
